#!/usr/bin/env python3
"""
Python implementation to construct input data for test_infiClSwap_ExactIn_SingleHop()
"""

"""该脚本构建的v4 cl流动性池的单个池子多跳路径输入参数，可以用于调用Universal Router的execute多跳功能。
对应测试函数：CLPancakeSwapInfinity.t.sol 的 test_infiClSwap_ExactIn_SingleHop
...
def construct_infi_cl_swap_exact_in_single_hop_input(
    token0_address, 
    token1_address, 
    ...
    """
    Construct input data for test_infiClSwap_ExactIn_SingleHop()
    ...
    """
    # Create PathKey array (single hop: token0 → token1)
    path = [
        PathKey(
...
if __name__ == "__main__":
    ...
    commands, inputs = construct_infi_cl_swap_exact_in_single_hop_input(
